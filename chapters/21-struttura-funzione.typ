...
#nota()[
  ...
  #figure(
    cetz.canvas({
      ...
      circle((3.5, 3.7), radius: (3.5, 1.8), fill: yellow.lighten(50%), stroke: black + 1.5pt)

      // e₃ = {v₃, v₅, v₆} - ovale verde (semi-trasparente per vedere sovrapposizioni)
      circle((5, 2.2), radius: (2.3, 2.2), fill: green.lighten(20%).transparentize(40%), stroke: black + 1.5pt)

      // e₂ = {v₂, v₃} - ovale rosa (semi-trasparente per vedere sovrapposizioni)
      circle((4.5, 3.9), radius: (2, 1.1), fill: red.lighten(30%).transparentize(40%), stroke: black + 1.5pt)

      // e₄ = {v₄} - cerchio viola piccolo
      circle((1, 0.8), radius: 1.0, fill: purple.lighten(60%), stroke: black + 1.5pt)
...
#esempio()[
  #figure(
    cetz.canvas({
      ...
      for i in range(5) {
        rect(
          (x-start + i * cell-width, y-array - 0.3),
      ...
      for (i, val) in b-values.enumerate() {
        ...
        rect(
          (x-start + i * cell-width, y-b - 0.3),
          (x-start + (i + 1) * cell-width, y-b + 0.3),
          stroke: black + 1pt,
          fill: fill-color,
        )
        content((x-start + i * cell-width + cell-width / 2, y-b), text(size: 9pt, str(val)))
      }
      ...
      content((3, -1.5), text(size: 10pt, $(x_(h_0(s)) + x_(h_1(s)) + x_(h_2(s))) mod 2^r$))
    }),
    caption: [
